		...
		$('#btn-contact').click(function(){
			...
			if (name == "") {
				...
				$.ajax({
					...
					type:"POST",
					url: "http://localhost/mi_portafolio/wordpress/wp-content/themes/PortafolioPersonal/mail.php",
					success: function(response){
						//console.log(response);
							...
							$('.ajaxgif').hide();
							$('.msg').text('Hubo un Error1!').animate({'right':'130px'}, 300);
						}
					...
					error: function(){
						$('.ajaxgif').hide();
						$('.msg').text('Hubo un Error2!').addClass('msg_error').animate({'right':'130px'}, 300);
